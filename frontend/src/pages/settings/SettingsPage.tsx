...
import { Bell, ChevronRight, LayoutDashboard, Palette, ShieldCheck, Sparkles, UserRound } from "lucide-react";
...
const SettingsPage = () => {
  ...
  return <main className="h-full overflow-y-auto px-4 pb-28 pt-[calc(env(safe-area-inset-top)+1rem)] md:pb-8"><div className="mx-auto max-w-xl">
    ...
    {isAdmin && <section className="mb-6"><p className="mb-2 px-1 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Admin</p><Link to="/admin" className="flex min-h-[76px] items-center gap-3 rounded-3xl border border-emerald-500/30 bg-card px-4 hover:bg-secondary/50"><span className="grid size-10 place-items-center rounded-2xl bg-emerald-500/15 text-emerald-400"><LayoutDashboard className="size-5" /></span><span className="min-w-0 flex-1"><span className="block font-semibold">Admin dashboard</span><span className="block truncate text-sm text-muted-foreground">Songs, users and comments</span></span><ChevronRight className="size-5 text-muted-foreground" /></Link></section>}
