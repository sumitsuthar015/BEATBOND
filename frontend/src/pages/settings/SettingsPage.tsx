...
const SettingsPage = () => {
  ...
  return <main className="h-full overflow-y-auto px-4 pb-28 pt-[calc(env(safe-area-inset-top)+1rem)] md:pb-8"><div className="mx-auto max-w-xl">
    ...
    <section className="mb-6 flex items-center gap-3 rounded-3xl border bg-card p-4"><Avatar className="size-14"><AvatarImage src={user?.imageUrl} /><AvatarFallback>{user?.firstName?.[0] || "U"}</AvatarFallback></Avatar><div className="min-w-0 flex-1"><p className="truncate font-semibold">{user?.fullName || "Your account"}</p><p className="truncate text-sm text-muted-foreground">{user?.primaryEmailAddress?.emailAddress}</p></div><EditProfileDialog /></section>
