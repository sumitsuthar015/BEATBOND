...
const listeningDetails = (activity?: string | null) => {
  ...
  return match
    ? { song: match[1], artist: match[2] }
    : { song: activity, artist: null };
};
...
export function FriendActivityProfileDialog({
  ...
  const { data: profile, isLoading } = useQuery<FriendProfile>({
    ...
    queryFn: async () =>
      (await axiosInstance.get(`/users/profile/${friend!.clerkId}`)).data,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md overflow-hidden p-0">
        <div className="h-28 bg-gradient-to-br from-primary/80 via-primary/50 to-violet-500/50" />
        <div className="px-6 pb-6">
          <div className="-mt-11 flex items-end justify-between gap-3">
            ...
            <span
              className={`mb-1 inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-medium ${isOnline ? "bg-emerald-500/15 text-emerald-600 dark:text-emerald-400" : "bg-muted text-muted-foreground"}`}
            >
              <span
          ...
          <DialogHeader className="mt-4 text-left">
            ...
            <DialogDescription>
              {profile?.username ? `@${profile.username}` : "Your friend"}
            </DialogDescription>
          </DialogHeader>
          ...
          {isLoading ? (
            ...
            <div className="mt-5 flex items-center gap-3 rounded-xl bg-muted/60 p-4 text-sm text-muted-foreground">
              ...
              {isOnline
                ? "Online, but not listening to music right now"
          ...
          <div className="mt-4 rounded-xl bg-muted/50 p-3">
            ...
            <p className="mt-1.5 line-clamp-3 text-sm leading-6 text-foreground/85">
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            <button
              ...
              onClick={() => {
                onOpenChange(false);
                navigate(`/profile/${friend.clerkId}/friends`);
              }}
              className="rounded-xl border bg-background/50 p-3 text-left transition-colors hover:bg-muted"
            >
            ...
            <div className="rounded-xl border bg-background/50 p-3">
              <span
                className={`mb-2 block h-4 w-4 rounded-full ${isOnline ? "bg-emerald-500" : "bg-muted-foreground"}`}
              />
          ...
          <div className="mt-5 grid grid-cols-3 gap-3">
            ...
            >
              <Link to={`/profile/${friend.clerkId}`}>
            ...
            <ShareToMessageDialog
              ...
              sharedContent={{
                ...
                subtitle: profile?.bio || "BeatBond profile",
